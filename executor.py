...
class Executor:
    def __init__(self, config):
        ...
        for judge_config in config["judge_configs"]:
            self.analyzer.register_origin(judge_config["name"])
    ...
    def start(self):
        ...
        for judge_config in self.judge_configs:
            ...
            result_judge_files_path = result_files_path / judge_config["name"]
            ...
            for file_index in range(sourcecode_files.__len__()):
                ...
                with open(info_path, "w") as info:
                    print("Test  " + str(file_index + 1) + "\tSourcecode: " + sourcecode_name, end="\t", flush=True)
                    ...
                    if execute_result == StatusCode.EXECUTE_OK:
                        # 正常执行，进入评判
                        compiler_output_path = compiler_output_files_path / judge_config["compiler_output_file"]
                        judge_statuscode, judge_info_object = judge_config["judge"].judge(compiler_output_path,
                                                                                          input_path,
                                                                                          answer_path)
                        self.analyzer.analyze(judge_statuscode, judge_config["name"], judge_info_object)
                        if judge_statuscode == StatusCode.JUDGE_AC:
                            print(judge_config["name"] + "=>" + GREEN + "AC" + RESET, end="")
                            if judge_info_object["info"] != "":
                                print("(" + judge_info_object["info"] + ")", end="\t")
                            else:
                                print("", end="\t")
                            info.writelines([">>> " + judge_config["name"] + "\n>>> ACCEPTED\n", "\n"])
                            ...
                            print(judge_config["name"] + "=>" + RED + "WA" + RESET, end="\t")
                            info.writelines([">>> " + judge_config["name"] + "\n>>> WRONG ANSWER\n", "\n"])
                            info.writelines([">>> Execute stdout\n", execute_stdout, "\n"])
                            info.writelines([">>> Judge Message\n", judge_info_object["info"], "\n"])
                        else:
                            print(judge_config["name"] + "=>" + "UNKNOWN", end="\t")
